...
def compute_tdb_energy_nc(sim, temps, c, phase):
    ...
    GM = pyc.calculate(sim._tdb, sim._components, phase, P=101325, T=flattened_t, points=fec_n_comp,
                       broadcast=False).GM.values.reshape(c[0].shape)
    GM_derivs = []
    for i in range(len(c)):
        GM_derivs.append((pyc.calculate(sim._tdb, sim._components, phase, P=101325, T=flattened_t,
                                        points=fec_nc_offset[i], broadcast=False).GM.values.reshape(
            c[0].shape) - GM) * (10000000.))
    return GM, GM_derivs
...
def gaq(gql, gqr, rgqsl, rgqsr, dqc, dx, dim):
    r = np.zeros_like(dqc)
    for i in range(dim):
        r += ((0.5 * (dqc + np.roll(dqc, -1, i)) * gqr[i] / rgqsr[i]) - (
                    0.5 * (dqc + np.roll(dqc, 1, i)) * gql[i] / rgqsl[i])) / (dx)
    return r
...
def NComponent(sim):
    ...
    vertex_centered_mgphi2 = vertex_averaged_gphi[0] * vertex_averaged_gphi[0] + vertex_averaged_gphi[1] * \
    ...
    pf_comp_x = 8 * sim.y_e * T * (
                (2 * a2_b2 * psix3 + 2 * ab2 * psiy3) / vertex_centered_mgphi2 - vertex_averaged_gphi[0] * (
    ...
    pf_comp_y = 8 * sim.y_e * T * (
                (2 * a2_b2 * psiy3 - 2 * ab2 * psix3) / vertex_centered_mgphi2 - vertex_averaged_gphi[1] * (
                    psix3 * vertex_centered_gpsi[0] + psiy3 * vertex_centered_gpsi[1]) / (
                            vertex_centered_mgphi2 * vertex_centered_mgphi2))
    pf_comp_y = (np.roll(pf_comp_y, -1, 1) - pf_comp_y) / dx
    pf_comp_y = (np.roll(pf_comp_y, -1, 0) + pf_comp_y) / 2.
    deltaphi = M_phi * (sim.ebar * sim.ebar * ((1 - 3 * sim.y_e) * divTgradphi + pf_comp_x + pf_comp_y) - 30 * g * (
                G_S - G_L) / sim.v_m - well - 4 * sim.H * T * phi * rgqs_0 * 1574.)
    ...
    t1_temp = (16 * sim.ebar * sim.ebar * T * sim.y_e / vertex_centered_mgphi2) * (
                psi_xyy * (q1px - q4py) + psi_xxy * (q1py + q4px))
    t4_temp = (16 * sim.ebar * sim.ebar * T * sim.y_e / vertex_centered_mgphi2) * (
                psi_xyy * (-q4px - q1py) + psi_xxy * (-q4py + q1px))
...
def init_NComponent(sim, dim=[200, 200], sim_type="seed", number_of_seeds=1, tdb_path="Ni-Cu_Ideal.tdb",
    ...
    if (thermal_type == "isothermal"):
        ...
        print(
            "thermal type of " + thermal_type + " is not recognized, defaulting to isothermal with a temperature of " + str(
                initial_temperature))
        sim.set_thermal_isothermal(initial_temperature)
    if (sim_type == "seed"):
