...
def get_thermodynamics(ufunc, array):
    #i dont know how to get cuda to send a variable number of arguments to a function, so...
    if(len(array) == 3):
        G = ufunc(array[0], array[1], array[2])
    elif(len(array) == 4):
        G = ufunc(array[0], array[1], array[2], array[3])
    elif(len(array) == 5):
        G = ufunc(array[0], array[1], array[2], array[3], array[4])
    elif(len(array) == 6):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5])
    elif(len(array) == 7):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5], array[6])
    elif(len(array) == 8):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7])
    elif(len(array) == 9):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7], array[8])
    elif(len(array) == 10):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7], array[8], array[9])
    elif(len(array) == 11):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7], array[8], array[9], array[10])
    elif(len(array) == 12):
        G = ufunc(array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7], array[8], array[9], array[10], array[11])
    return G
...
def NComponent_helper_kernel(fields, T, transfer, rng_states, ufunc_array, params, c_params):
    ...
    for i in range(startx, fields[0].shape[0], stridex):
        for j in range(starty, fields[0].shape[1], stridey):
            ...
            transfer[0][i][j] = get_thermodynamics(ufunc_g_l, ufunc_array[i][j])
            transfer[1][i][j] = get_thermodynamics(ufunc_g_s, ufunc_array[i][j])
            ...
            for l in range(3, len(fields)):
                ...
                dGLdc = 10000000.*(get_thermodynamics(ufunc_g_l, ufunc_array[i][j]) - transfer[0][i][j])
                dGSdc = 10000000.*(get_thermodynamics(ufunc_g_s, ufunc_array[i][j]) - transfer[1][i][j])
...
def init_tdb_parameters(sim):
    ...
    try:
        ...
        model_solid = pyc.Model(sim._tdb, comps, "FCC_A1")
        ...
        model_liquid = pyc.Model(sim._tdb, comps, "LIQUID")
...
def init_NCGPU(sim, dim=[200,200], sim_type="seed", number_of_seeds=1, tdb_path="Ni-Cu_Ideal.tdb", 
